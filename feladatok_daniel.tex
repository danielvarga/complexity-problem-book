...
A nyelvosztályokra készített makróm csúnyán bánik a kötőjelekkel: A legcsúnyább a \coNP, és pár másik co-, rájuk kell keresni. Bezzeg a \la{co-NP}-vel semmi baj.
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Ha az \Language nyelv \NP-teljes, akkor az $\Language' = \{ xx : x \in \Language \}$ nyelv is az.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Adjunk polinomiális algoritmust egy diszjunktív normálforma kielégíthetőségének eldöntésére.
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Jelölje \la{DOUBLE-SAT} a legalább két különböző kielégítéssel rendelkező konjunktív
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy A lineáris időben kiszámítható Karp-redukcióra nézve nem létezik \P-teljes nyelv.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy \reff{Friedl 9.13.} Pontos 3-fedésnek (exact 3-cover, \la{X3C}) hívjuk annak eldöntését, hogy egy háromelemű halmazokból álló halmazrendszernek létezik-e olyan részhalmazrendszere, amely partíció. Abból a feltételezésből kiindulva, hogy az \la{X3C} eldöntési feladat megoldására létezik polinomidejű eljárásunk, adjunk olyan eljárást, amely polinomidőben megoldja az \la{X3C} keresési feladatot, azaz adott halmazrendszerhez megkeresi a megfelelő partíciót, ha az létezik.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy \reff{Friedl 9.30., 9.31.} Az $n$ csúcsú, $n+10$ élű gráfokon a \la{HAMILTON} és a 3-színezhetőség eldöntése polinomidőben lehetséges.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy \reff{Friedl 9.37.} \NP-teljes annak eldöntése, hogy egy $n$ csúcsú gráfban létezik-e legalább $\sqrt n$ hosszú kör.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Véges nyelv felismerhető konstans időben.
\end{Exercise}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Adjunk \LOGSPACE algoritmust, ami eldönti, hogy egy szó palindróma-e.
\end{Exercise}
...
\sol{ Először ellenőrizzük le logaritmikus tárban mindkét fajta zárójelről, hogy a másik fajta zárójelektől elvonatkoztatva, önmagában helyesen zárójelezett-e. (Ehhez oldjuk meg az előző feladatot.) Ha igen, akkor minden (nyitó- vagy záró, gömbölyű vagy szögletes) zárójelnek egyértelműen létezik párja, és az logaritmikus memóriával meg is található. Ekkor a következő egyszerű struktúratételt mondhatjuk ki: A szó akkor és csak akkor helyesen zárójelezett, ha nem létezik két olyan, különböző típusú zárójel-pár, amelyek ,,egymásba gabalyodottak'', azaz $(\ldots[\ldots)\ldots]$ mintájúak. Az ilyen tiltott konfigurációk megkereséséhez pedig egyszerűen végigmehetünk egy kettős ciklusban minden lehetséges olyan páron, ami két különböző típusú nyitó zárójelből áll, és a párjaikat megkeresve megvizsgálhatjuk az ,,egymásba gabalyodottságot''. \wrk{Egyrészt ez kicsit pongyola. Másrészt homályban hagyja, hogy a helyes zárójelezettség miféle definíciójából, és hogyan vezettük le a struktúratételt.} }
...
\hint{ \wrk{Mondani se kell, hogy ez még nincs kész.} A redukció az alábbi sorokból fog állni: A DAG minden $x$ csúcsára leírjuk $x$-et kétszer, majd felsoroljuk a szomszédait. $x$ DELIMITER1 $x$ D2 $y_1$ D3 $y_2$ D3 \ldots D3 $y_k$ D4. Ezeket az izéket összefűzzük abban a topologikus sorrendben, amelyben a gráf csúcsai adva vannak. Az \Language nyelvben olyan mondatok vannak, amelyekre igaz, hogy az első szó szomszédai között szerepel az utolsó szó: $x$ D2 $y_1$ D3 \ldots $y_l$ \ldots D3 $y_k$ D4, aztán teljes izéből jópár, aztán a végén D4 $y_l$ D1. }
\end{Exercise}
...
\definition{ $\la{MAJORITY}(x_1,\ldots,x_n) = \lfloor \sum_i{x_i} / 2 \rfloor$. \wrk{Kicsit faramuci, hogy pont most vezetem be a MAJORITY-t.} }
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Annak eldöntése, hogy két Branching Program ugyanazt a függvényt számítja-e ki, pontosan
...
Adott egy \SAT formula az $x_1,\ldots,x_n$ változókkal. Kössük le a változókat
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Egy nyelv ritka, ha $n$ betűs szavainak száma $poly(n)$. A ritka nyelvek osztályát \SPARSE jelöli. \P-közelinek nevezünk egy \Language nyelvet, ha valamely $\Language' \in \P$ nyelvvel vett szimmetrikus differenciája ritka. \cl{P-close} a \P-közeli nyelvek osztálya. Bizonyítsuk be, hogy \cl{P-close} valódi része \Ppoly-nak.
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy $| K_{Java}(x) - K_{C++}(x) | \leq c_{Java,C++}$. \wrk{Kati szerint pongyola ez a jelölés.}
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy $P_{|x|=n}(K(x) \geq |x|-c) \geq 1-1/{2^c}$.
\end{Exercise}


\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Létezik olyan $n$ szám, hogy az $n$-nél nagyobb prímek között nincsenek
Kolmogorov-véletlenek.
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Létezik olyan $n$ szám, hogy a $\pi$ $n$-nél hosszabb kettes számrendszerbeli
...
\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Minden elegáns program Kolmogorov-véletlen.
\end{Exercise}
...
\sol{ Egy $n$ szám $m$ modulus szerint vett négyzetét természetesen ki tudjuk számolni polinomidőben. A négyzetre emelés műveletét ismételgetve a negyedik, nyolcadik, stb. hatványát is ki tudjuk számolni polinomidőben, amíg a négyzetre emelések összes száma polinomiális. $t$ kettes számrendszerbeli felírása legyen $2^{i_1}+\ldots+2^{i_k}$. Ez alapján $n^t = n^{2^{i_1}+\ldots+2^{i_k}} = \prod_l{n^{2^{i_l}}}$. Ennek a szorzatnak minden tényezője kiszámolható polinomidőben, és összesen polinomiális számúan vannak. }


\begin{Exercise}[counter={sorszam}, difficulty=0]
\easy Egy $n$ szám $t$-edik hatványát nem tudjuk kiszámolni polinomidőben az egészek között.
\end{Exercise}
